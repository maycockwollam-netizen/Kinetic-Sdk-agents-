...
class LLMContextSummarizer:
    ...
    def summarize(self, messages: list[Message]) -> str:
        ...
        response = self.client.chat(messages=prompt, max_tokens=self.max_tokens)
        return (response.content or "").strip()

    def _build_prompt(self, messages: list[Message]) -> list[Message]:
        ...
        return [
            ...
            {"role": "user", "content": f"Đoạn hội thoại cần tóm tắt:\n{body}"},
        ]
...
class SummarizingContextManager(SimpleTruncateContextManager):
    ...
    def compact(self, state: ConversationState) -> ConversationState:
        """Compact by summarizing the elided span when possible."""
        if self.summarizer is None:
            return super().compact(state)
        ...
        tail_start = self._tail_start(messages)
        head_end = 1 if tail_start > 0 else 0
        removed = tail_start - head_end
        if removed <= 0:
            return self._copy(state, list(messages))

        summary = self._summarize(messages[head_end:tail_start])
        if not summary:
            return super().compact(state)
    ...
    def _summarize(self, messages: list[Message]) -> str:
        try:
            if self.summarizer is None:
                return ""
            summary = self.summarizer.summarize([dict(m) for m in messages])
